...
Question 9.3.13
\\
Find the probablity of getting 5 twice in 7 throws of a dice.
\solution\\
\begin{table}[ht]
    \centering
    \caption{Variables}
...
\hline
n & 7 & Number of throws\\
\hline
p & $\frac{1}{6}$ & probability of getting five\\
\hline
$\mu=np$ & $\frac{7}{6}$ & mean of distribution\\
\hline
$\sigma=\sqrt{npq}$ & $\sqrt{\frac{35}{216}}$ & variance of distribution\\
\hline
$X$ & $0 \le X \le 7$ & Number of correct questions \\
\hline
\end{tabular} 
\end{table}
\textbf{Gaussian:}
...
\pr{Y = 2} &= 1-\pr{Y = 2}\\
&= 1-F_Y\brak{4}\\
\implies \pr{Y = 2} &= \qfunc{\frac{X-\mu}{\sigma}}\\
&= \qfunc{2.07036}\\
\pr{Y = 2} &= 0.019209
\end{align}
\item With a 0.5 correction:
\begin{align}
\pr{Y = 2} &= \qfunc{\frac{X-\mu+0.5}{\sigma}}\\
&= \qfunc{0.82824}\\
\implies \pr{Y = 2} &= 0.20377
\end{align}
\end{enumerate}
\textbf{Binomial:}
\begin{align}
...
\begin{align}
	\implies P\brak{X = 2}&=\sum_{k=2}^{5}\comb{7}{k}\brak{\frac{1}{6}}^k\brak{\frac{5}{6}}^{7-k}\\
	&=0.234427244
	\end{align}
\begin{figure}[h]
